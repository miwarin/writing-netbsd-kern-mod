...
int rperm_open(dev_t self, int flag, int mod, struct lwp* l)
{
        return 0;
}

int rperm_close(dev_t self, int flag, int mod, struct lwp* l)
{
        if(sc.buf != NULL)
        {
...
int rperm_write(dev_t self, struct uio* uio, int flags)
{
        if(sc.buf != NULL)
        {
                kmem_free(sc.buf, sc.buf_len);
...
int rperm_read(dev_t self, struct uio* uio, int flags)
{
...
static int rperm_modcmd(modcmd_t cmd, void* args)
{
